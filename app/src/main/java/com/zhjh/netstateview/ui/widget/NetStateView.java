package com.zhjh.netstateview.ui.widget;
...
import com.zhjh.netstateview.utils.UIUtils;
...
public class NetStateView extends LinearLayout {
    ...
    public NetStateView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.styleNetworkStateView);
    }

    public NetStateView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NetStateView, defStyleAttr, R.style.NetworkStateView_Style);

        mLoadingViewId = typedArray.getResourceId(R.styleable.NetStateView_loadingView, R.layout.view_loading);

        mErrorViewId = typedArray.getResourceId(R.styleable.NetStateView_errorView, R.layout.view_net_error);
        mErrorImageId = typedArray.getResourceId(R.styleable.NetStateView_nsvErrorImage, NO_ID);
        mErrorText = typedArray.getString(R.styleable.NetStateView_nsvErrorText);

        mNoNetworkViewId = typedArray.getResourceId(R.styleable.NetStateView_noNetworkView, R.layout.view_no_network);
        mNoNetworkImageId = typedArray.getResourceId(R.styleable.NetStateView_nsvNoNetworkImage, NO_ID);
        mNoNetworkText = typedArray.getString(R.styleable.NetStateView_nsvNoNetworkText);

        mEmptyViewId = typedArray.getResourceId(R.styleable.NetStateView_emptyView, R.layout.view_empty);
        mEmptyImageId = typedArray.getResourceId(R.styleable.NetStateView_nsvEmptyImage, NO_ID);
        mEmptyText = typedArray.getString(R.styleable.NetStateView_nsvEmptyText);

        mRefreshViewId = typedArray.getResourceId(R.styleable.NetStateView_nsvRefreshImage, NO_ID);

        mTextColor = typedArray.getColor(R.styleable.NetStateView_nsvTextColor, 0x8a000000);
        mTextSize = typedArray.getDimensionPixelSize(R.styleable.NetStateView_nsvTextSize, UIUtils.dp2px(14));
